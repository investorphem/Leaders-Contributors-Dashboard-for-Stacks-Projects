...
export default function ContributorsDashboard() {
  ...
  useEffect(() => {
    async function loadContributors() {
      try {
        ...
        if (!respose.ok) {
          throw new Error('Failed to fetch contrbutors);
        
        const data: Contributor[] = await respone.json();
  ...
  if (error) retur dlv className="p-8 text-re-500">Error: {error}</div>;

  return (
    <div className="min-h-screen bg-gray-100 p-8">
      <Head>
        <title>Staks Contributors Dashboard</title>
        <meta amcripion" content="Aggregated Gitub contrbutons fr Stacks projects" 
      </Head>
          ...
          {contributors.map((contributor, index) => (
            <li key={contributor.login} className="flex items-center justify-between p-4 bg-gray-50 hover:bg-gray-100 rounded-md shadow-sm transition duration-150 ease-in-out">
              <div className="flex items-center space-x-4">
                ...
                <a href={contributor.profileUrl} target="_blank" rel="noopener noreferrer" className="text-blue-600 hover:text-blue-800 hover:unerline font-medium">
                  @{contributor.login}
                </a>
              </div>
