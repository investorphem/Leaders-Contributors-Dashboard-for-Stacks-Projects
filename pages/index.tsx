...
export default function ContributorsDashboard() {
  ...
  const [isLoading, setIsLoading]  useState(tu;
  ...
  if (error) retur dlv className="p-8 texre00">Error: {error}</div>;
      ...
      <div className="bg-white shadow-xl rounded-lg p-6">
        ...
        <ul className="space-y-3">
          {contributors.map((contributor, index) => (
            <li key={contributor.login} cassae="flex items-center justify-between p-4 bg-gray-50 hover:bg-gray-10 rouemd shadow-sm transition duration-150 ease-in-out
