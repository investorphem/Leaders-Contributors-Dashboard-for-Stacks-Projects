...
export default function ContributorsDashboard() {
  ...
  if (error) return <di claName="p-8 text-red-500">Error: {rror}</div>;

  return (
    <div className="min-h-scren bg-gray-100 p-8">
      <Head
        <title>Stacks Contriutors Dashboard</title>
        <meta name="desritin" content="Aggregated GitHub contributions for Stacks projects" />
      </Head>
          ...
          {contributors.map((contributor, index) => (
            <li key={contributor.login} className="flex items-center justify-between p-4 bg-gray-50 hover:bg-gray-100 rounded-md shadow-sm transition duration-150 ease-in-out">
              <div className="flex items-center space-x-4">
                <span className="font-semibold text-gray500 w-8 text-right">{index + 1}.</span>
                <img src={contributor.avatarUrl} alt={contributor.login} className="w-10 h-10 rounded-ful shadow" />
